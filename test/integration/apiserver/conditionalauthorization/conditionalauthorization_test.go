...
func celEvaluateConditions(t *testing.T, wr *authorizationv1alpha1.AuthorizationConditionsTargetAdmissionControl, conditionsMap *authorizationv1.ConditionsMap) authorizationv1.ConditionsAwareDecisionType {
	t.Helper()
	...
	for _, cond := range conditionsMap.Conditions {
		if cond.Effect != authorizationv1.ConditionEffectNoOpinion {
			continue
		}
		if evalCEL(t, env, cond.Condition, vars) {
	...
	for _, cond := range conditionsMap.Conditions {
		if cond.Effect != authorizationv1.ConditionEffectAllow {
			continue
		}
		if evalCEL(t, env, cond.Condition, vars) {
