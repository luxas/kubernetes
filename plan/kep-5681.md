...
## Appendix C: Implementation Notes — Deviations from the Proposal

This section documents how the actual implementation (on the `lean-test` branch)
deviates from or refines the proposal above. These changes were made during
implementation to improve ergonomics, backwards compatibility, and API clarity.

### Core interface: additive methods instead of replacing `Decision`

The proposal suggested replacing the existing `authorizer.Decision` int enum
with a new struct type `Decision` (with private fields and constructors like
`DecisionAllow(reason string)`), and changing the `Authorize` return signature
accordingly.

The implementation instead **keeps the existing `Decision` int enum and
`Authorize()` signature entirely unchanged**, and introduces a new, separate
`ConditionsAwareDecision` struct alongside it. The `Authorizer` interface gains
...
type Authorizer interface {
    ...
    // New: returns a ConditionsAwareDecision (which embeds reason+error)
    ConditionsAwareAuthorize(ctx context.Context, a Attributes) ConditionsAwareDecision

    // New: evaluates a conditional decision against previously-unknown data
...
Non-conditions-aware authorizers implement `ConditionsAwareAuthorize` as a
one-liner: `return ConditionsAwareDecisionFromParts(self.Authorize(ctx, a))`,
and `EvaluateConditions` as
`return DecisionDeny, "", ErrorConditionEvaluationNotSupported`.
An `AuthorizerFunc` adapter is provided for this.

The KEP's proposed `ConditionSetEvaluator` interface was merged into the
`Authorizer` interface directly (as the `EvaluateConditions` method), rather
than being a separate composable interface.

### Five-variant decision enum instead of four

The proposal modelled the decision as four variants:
`Allow | Deny | NoOpinion | Conditional(ConditionSet)`.

The implementation splits `Conditional` into two distinct variants, yielding
five variants:

1. **Allow** — unconditional allow
2. **Deny** — unconditional deny
3. **NoOpinion** — unconditional no opinion
4. **ConditionsMap** — a single authorizer's conditional decision (leaf node)
5. **Union** — an ordered tree of sub-decisions from multiple authorizers
   (internal node)

This makes the tree structure of composite/union authorizer decisions explicit
in the type system. The `ConditionsAwareDecision` struct uses private fields,
and exactly one of `IsAllowed()`, `IsDenied()`, `IsNoOpinion()`,
`IsConditionsMap()`, or `IsUnion()` is true at any time.

Constructors:

- `ConditionsAwareDecisionDeny(reason string, err error)`
- `ConditionsAwareDecisionAllow(reason string, err error)`
- `ConditionsAwareDecisionNoOpinion(reason string, err error)`
- `ConditionsAwareDecisionFromParts(Decision, string, error)` — bridge from old
  triple
- `ConditionsAwareDecisionConditionsMap(conditions ...Condition)` — validates
  conditions, enforces feature gate
- `ConditionsAwareDecisionUnion(decisions ...ConditionsAwareDecision)` — builds
  tree with simplification

### `ConditionsMap` replaces `ConditionSet`

The proposal used the term "ConditionSet" with a `Type() string` method on the
set and a flat `set []Condition` field.

The implementation uses the term **`ConditionsMap`** instead, with conditions
pre-sorted into three buckets by effect:
...
The `Type` field moved from the map-level to **individual `Condition`s** (each
condition has its own `GetType() string`). This allows a single `ConditionsMap`
to contain conditions of heterogeneous types.

A maximum of `MaxConditionsPerMap = 128` conditions is enforced per map.

### `Condition` is an interface (not a struct) in the Go authorizer package

The proposal defined `Condition` as a struct with public fields. The
implementation defines `Condition` as an **interface**:
...
This allows authorizers to use pre-compiled conditions (e.g. a pre-compiled CEL
program) that can be evaluated efficiently without re-parsing, via the
`Evaluate()` method. A `GenericCondition` struct implementing this interface is
provided for simple cases.

The `ConditionEvaluationResult` type is a four-variant enum:
`true | false | error | unevaluatable`, where "unevaluatable" signals that the
condition cannot be evaluated in-process and needs to be sent to the authorizer
via a webhook.

### `ConditionsMode` replaced by `ConditionalAuthorizationOptions.Enabled`

The proposal added a `ConditionsMode` enum (`None | HumanReadable | Optimized`)
to the `Attributes` interface, controlling how conditions should be returned.

The implementation **does not add `ConditionsMode` to `Attributes`**. Instead,
the `SubjectAccessReviewSpec` gains a simpler field:
...
The `HumanReadable` vs `Optimized` presentation modes were dropped for the
initial implementation.

### No lazy authorizer chain references in `Decision`

The proposal included `ChainedDecisions(decision []Decision,
authorizerChain ...Authorizer) Decision`, which would lazily capture remaining
authorizers in the chain for deferred evaluation. It also proposed
`AllConditionSets(ctx) ([]*ConditionSet, error)` and
`Evaluate(ctx, data, builtinEvaluators...) (Decision, error)` as methods on
`Decision`.

The implementation takes a different approach:

- `ConditionsAwareDecisionUnion(decisions ...ConditionsAwareDecision)` builds the
  tree eagerly from already-computed decisions (no lazy authorizer references).
- Evaluation is done via `ConditionsMap.Evaluate(ctx, data, evaluateFunc)` on
  the map level, and `Authorizer.EvaluateConditions(ctx, decision, data)` on the
  authorizer level.
- There is no `BuiltinConditionSetEvaluator` interface yet; the evaluation
  function is passed directly.

### `SubjectAccessReviewStatus` uses `ConditionalDecision` instead of `ConditionSetChain`

The proposal added a `ConditionSetChain []SubjectAccessReviewConditionSet`
field, where each element had `AuthorizerName`, `ConditionsType`, `Conditions`,
`ConditionSetChain` (nested), `Allowed`, `Denied`, and `FailureMode` fields.

The implementation uses a simpler, single field:

```go
type SubjectAccessReviewStatus struct {
    // ... Allowed, Denied, Reason, EvaluationError as before ...

    // New: the full conditional decision tree
    ConditionalDecision *ConditionsAwareDecision `json:"conditionalDecision,omitempty"`
}
```

The `ConditionsAwareDecision` API type is a discriminated union with a `Type`
enum field (`Deny | Allow | NoOpinion | ConditionsMap | Union`), which
recursively nests via the `Union []ConditionsAwareDecision` field. This replaces
the chain-based model with a tree-based model.

Notably absent from the implementation compared to the KEP's
`SubjectAccessReviewConditionSet`:

- **`AuthorizerName`** — not included in the decision type. Routing of
  conditions back to the correct authorizer for evaluation is handled
  internally by the union authorizer walking its sub-authorizer list, not
  by a name field in the API.
- **`ConditionsType`** — moved to individual `Condition.Type` instead of
  being at the set level.
- **`FailureMode`** — not included; fail-closed semantics are hardcoded
  (Deny conditions that error → Deny, NoOpinion conditions that error →
  NoOpinion, Allow conditions that error → ignored/NoOpinion).

### `AuthorizationConditionsReview` API structure

The proposal sketched the `AuthorizationConditionsReview` request with a
`ConditionSetChain` field plus flat condition data fields. The response had
`Allowed`, `Denied`, `Reason`, `EvaluationError`, and `ConditionSetChain`.

The implementation (in `authorization/v1alpha1`) structures it differently:

```go
type AuthorizationConditionsRequest struct {
    Decision             ConditionsAwareDecision                        `json:"decision"`
    AdmissionControlData *AuthorizationConditionsTargetAdmissionControl `json:"admissionControlData,omitempty"`
}
...
The `AuthorizationConditionsTargetAdmissionControl` struct closely follows the
`AdmissionReview` structure (with `RequestKind`, `RequestResource`,
`RequestSubResource`, `Name`, `Namespace`, `Operation`, `UserInfo`, `Object`,
`OldObject`, `DryRun`, `Options`), rather than using a flat set of fields.

The response contains just a `Decision` (the full `ConditionsAwareDecision`
type), which can represent the evaluated concrete result or a partially-
evaluated residual.

### `WithAuthorization` filter: separate function, `Attributes`-based classifier

The proposal augmented `WithAuthorization` with a
`supportsAuthorizationConditions func(ctx context.Context)` parameter.

The implementation adds a **separate function**
`WithAuthorizationAndConditionsSupport` alongside the existing
`WithAuthorization` (which remains unchanged):

```go
type ConditionalAuthorizationRequestClassifier func(attrs authorizer.Attributes) bool

func WithAuthorizationAndConditionsSupport(
...
The classifier takes `Attributes` (not `context.Context`), giving it access to
verb, resource, API group, etc., to decide whether the request class supports
conditions.

### Webhook `AuthorizationConfiguration` field naming

The proposal used `conditionsEndpointKubeConfigContext` and
`authorizationConditionsReviewVersion` fields.

The implementation uses a nested struct instead:
...
### `ConditionsData` structure

The proposal sketched `ConditionData` as an interface with `GetOperation()`,
`GetOperationOptions()`, `GetObject()`, `GetOldObject()`.

The implementation uses `ConditionsData` (note the plural) as a **struct** with
a `ConditionsDataAdmissionControl` interface field:
...
The `ConditionsDataAdmissionControl` interface is a subset of
`admission.Attributes`, exposing: `GetName()`, `GetNamespace()`,
`GetResource()`, `GetSubresource()`, `GetOperation()`,
`GetOperationOptions()`, `IsDryRun()`, `GetObject()`, `GetOldObject()`,
`GetKind()`, `GetUserInfo()`. This is richer than what the KEP proposed, as
it includes fields like `GetUserInfo()` and `IsDryRun()` that admission
controllers commonly need.

### Context propagation

The implementation adds context helpers for propagating the conditional decision
through the request pipeline:

```go
func WithConditionallyAuthorizedDecision(parent context.Context, authorizer Authorizer,
    decision ConditionsAwareDecision) context.Context
func ConditionallyAuthorizedDecisionFrom(ctx context.Context) (Authorizer, ConditionsAwareDecision, bool)
```

Both the `Authorizer` and the `ConditionsAwareDecision` are stored in context,
so the admission plugin can call `EvaluateConditions` on the correct authorizer.
