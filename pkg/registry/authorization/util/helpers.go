...
func SARStatusFromAuthorize(ctx context.Context, authz authorizer.Authorizer, attrs authorizer.AttributesRecord, conditionalOpts *authorizationapi.ConditionalAuthorizationOptions) authorizationapi.SubjectAccessReviewStatus {
	...
	if !utilfeature.DefaultFeatureGate.Enabled(genericfeatures.ConditionalAuthorization) || conditionalOpts == nil {
	...
	var encodingPref authorizer.ConditionsEncodingPreference
	switch conditionalOpts.EncodingPreference {
	case authorizationapi.ConditionsEncodingPreferenceOptimized:
		encodingPref = authorizer.ConditionsEncodingPreferenceOptimized()
	case authorizationapi.ConditionsEncodingPreferenceHumanReadable:
		encodingPref = authorizer.ConditionsEncodingPreferenceHumanReadable()
	default:
		encodingPref = authorizer.ConditionsEncodingPreferenceOptimized()
	}

	decision := authz.AuthorizeConditionsAware(ctx, attrs, encodingPref)
	...
	case decision.IsConditionsMap():
		...
		for id, condition := range decision.ConditionsMap().Conditions() {
			conds = append(conds, authorizationapi.Condition{
				ID:          id,
				Effect:      authorizationapi.ConditionEffect(condition.Effect),
		...
		return authorizationapi.ConditionsAwareDecision{
			...
			ConditionsMap: &authorizationapi.ConditionsMap{
				...
				ConditionsTarget: authorizationapi.ConditionsTarget(decision.ConditionsMap().Target()),
			},
		}
