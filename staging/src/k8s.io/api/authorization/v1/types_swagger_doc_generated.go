...
var map_AuthorizationOptions = map[string]string{
	...
	"handledDecisionTypes": "handledDecisionTypes specifies what decision types the client can handle in the context it is in. Currently valid values are: - [Allow, Deny, NoOpinion] (for conditions-unaware clients) or - [Allow, Deny, NoOpinion, ConditionsMap, Union] (for conditions-aware clients) If the authorizer would like to return conditions, but the client does not opt in to handle those here, . the authorizer must fail closed to a safe unconditional decision (Deny if any Deny conditions were present, otherwise NoOpinion). Order does not matter in this slice; set semantics should be used. The server should intersect this set with its own supported decision types, and call the authorizer either in conditions-aware or -unaware mode based on that intersection. Note that this intersection is done outside of the normal validation phase, the server should make sure the set intersection is correct after normal validation (hence the k8s:opaqueType) tag is used to not fail validation on server-unsupported ConditionsAwareDecisionTypes in version skew scenarios.",
}
