...
// ConditionsEncodingPreference specifies how, if at all, the client wants conditions to be
// returned by a conditional authorizer. The default is ConditionsEncodingPreferenceOptimized.
// +enum
type ConditionsEncodingPreference string

const (
	// ConditionsEncodingPreferenceOptimized indicates that the client wants an
	// optimized conditions encoding without description, if possible.
	ConditionsEncodingPreferenceOptimized ConditionsEncodingPreference = "Optimized"

	// ConditionsEncodingPreferenceHumanReadable indicates that the client wants a
	// human-readable condition and description, if possible.
	ConditionsEncodingPreferenceHumanReadable ConditionsEncodingPreference = "HumanReadable"
)
...
type ConditionalAuthorizationOptions struct {
	// encodingPreference specifies how the client would prefer the authorizer to encode the conditions.
	// If unset, this field is defaulted to ConditionsEncodingPreferenceOptimized.
	// +optional
	EncodingPreference ConditionsEncodingPreference `json:"encodingPreference,omitempty" protobuf:"bytes,1,opt,name=encodingPreference"`
}
