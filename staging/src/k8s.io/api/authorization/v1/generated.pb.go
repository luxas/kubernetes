...
func (m *ConditionalAuthorizationOptions) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	i -= len(m.EncodingPreference)
	copy(dAtA[i:], m.EncodingPreference)
	i = encodeVarintGenerated(dAtA, i, uint64(len(m.EncodingPreference)))
	i--
	dAtA[i] = 0xa
	return len(dAtA) - i, nil
}
...
func (this *ConditionalAuthorizationOptions) String() string {
	...
	s := strings.Join([]string{`&ConditionalAuthorizationOptions{`,
		`EncodingPreference:` + fmt.Sprintf("%v", this.EncodingPreference) + `,`,
		`}`,
	}, "")
	return s
}
...
func (m *ConditionalAuthorizationOptions) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field EncodingPreference", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.EncodingPreference = ConditionsEncodingPreference(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipGenerated(dAtA[iNdEx:])
