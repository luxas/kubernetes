...
func ValidateSubjectAccessReviewCreate(ctx context.Context, scheme *runtime.Scheme, sar *authorizationv1.SubjectAccessReview, version string) field.ErrorList {
	...
	return dv.ValidateDeclaratively(withRequestInfo(ctx, version, "subjectaccessreviews"), sar, nil, errs, operation.Create, sarValidationConfig())
}
...
func ValidateSelfSubjectAccessReviewCreate(ctx context.Context, scheme *runtime.Scheme, sar *authorizationv1.SelfSubjectAccessReview, version string) field.ErrorList {
	...
	dv := rest.DeclarativeValidation{Scheme: scheme}
	return dv.ValidateDeclaratively(withRequestInfo(ctx, version, "selfsubjectaccessreviews"), sar, nil, errs, operation.Create, sarValidationConfig())
}
...
func ValidateLocalSubjectAccessReviewCreate(ctx context.Context, scheme *runtime.Scheme, sar *authorizationv1.LocalSubjectAccessReview, version string) field.ErrorList {
	...
	dv := rest.DeclarativeValidation{Scheme: scheme}
	return dv.ValidateDeclaratively(withRequestInfo(ctx, version, "localsubjectaccessreviews"), sar, nil, errs, operation.Create, sarValidationConfig())
}
...
//   - not all callers might be in the API server with RequestInfo already set up properly (e.g. webhook authorizers that want to perform proper validation),
//     and thus does taking the version as an explicit parameter this dependency explicit.
func ValidateAuthorizationConditionsReviewCreate(ctx context.Context, scheme *runtime.Scheme, acr *authorizationv1alpha1.AuthorizationConditionsReview, version string) field.ErrorList {
	errs := ValidateAuthorizationConditionsReview(acr)
	dv := rest.DeclarativeValidation{Scheme: scheme}
	return dv.ValidateDeclaratively(withRequestInfo(ctx, version, "authorizationconditionsreviews"), acr, nil, errs, operation.Create, sarValidationConfig())
}
