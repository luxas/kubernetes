	...
	tests := []struct {
		...
		// ============================================================
		{
			...
			subCases: []subCase{
				...
				{
					name: "deny match takes precedence over matching nop and allow",
					conditions: []authorizer.Condition{
						cond("deny-1", authorizer.ConditionEffectDeny, trueResult),
		...
		},
		{
			...
			wantString: `NoOpinion(reason="one or more conditional evaluation errors occurred", err="[Allow condition \"allow-1\" produced error: eval error, Allow condition \"allow-2\" produced error: eval error]")`,
			subCases: []subCase{
				{
					name: "minimal",
					conditions: []authorizer.Condition{
		...
		// ============================================================
		{
			...
			wantString: `Allow(reason="condition \"allow-1\" allowed the request", err="Allow condition \"allow-err\" produced error: eval error")`,
			subCases: []subCase{
				{
					name: "minimal",
					conditions: []authorizer.Condition{
		...
		t.Run(tt.name, func(t *testing.T) {
			for _, sc := range tt.subCases {
				t.Run(sc.name, func(t *testing.T) {
					...
					decision := authorizer.ConditionsAwareDecisionConditionMap(sc.conditions...)
...
func TestConditionsMapEvaluateDeepCopy(t *testing.T) {
	...
	decision := authorizer.ConditionsAwareDecisionConditionMap(
...
func (a sampleAuthorizer) EvaluateConditions(ctx context.Context, unevaluated authorizer.ConditionsAwareDecision, data authorizer.ConditionsData) (authorizer.Decision, string, error) {
	...
	if !unevaluated.IsConditionsMap() {
		return unevaluated.FailClosedDecision(errors.New("can only evaluate unconditional or ConditionsMap decisions")).UnconditionalParts()
	}
...
func celEvaluateConditions(ctx context.Context, conditionsMap authorizer.ConditionsMap, data authorizer.ConditionsData) (authorizer.Decision, string, error) {
	...
	if err != nil {
		return conditionsMap.FailClosedDecision(fmt.Errorf("failed to create CEL env: %v", err)).UnconditionalParts()
	}

	if data.AdmissionControl == nil {
		return conditionsMap.FailClosedDecision(errors.New("evaluating a CEL condition requires non-nil data.AdmissionControl")).UnconditionalParts()
	}

	obj, err := objectToResolveVal(data.AdmissionControl.GetObject())
	if err != nil {
		return conditionsMap.FailClosedDecision(fmt.Errorf("failed to convert object to CEL ref.Val: %v", err)).UnconditionalParts()
	}
...
func evalCEL(env *cel.Env, expr string, vars map[string]any) authorizer.ConditionEvaluationResult {
	...
	if issues != nil && issues.Err() != nil {
		return authorizer.ConditionEvaluationResultError(fmt.Errorf("CEL compile error for %q: %v", expr, issues.Err()))
	}
	prg, err := env.Program(ast)
	if err != nil {
		return authorizer.ConditionEvaluationResultError(fmt.Errorf("CEL program error for %q: %v", expr, err))
	}
	out, _, err := prg.Eval(vars)
	if err != nil {
		return authorizer.ConditionEvaluationResultError(fmt.Errorf("CEL eval error for %q: %v", expr, err))
	}
