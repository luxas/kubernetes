...
func celEvaluateConditions(ctx context.Context, conditionsMap authorizer.ConditionsMap, data authorizer.ConditionsData) (authorizer.Decision, string, error) {
	...
	if err != nil {
		return conditionsMap.FailClosedDecision(), "failed closed", fmt.Errorf("failed to create CEL env: %v", err)
	}
	...
	if err != nil {
		return conditionsMap.FailClosedDecision(), "failed closed", fmt.Errorf("failed to convert object to CEL ref.Val: %v", err)
	}
