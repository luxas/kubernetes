...
func (unionMap *ConditionsAwareDecisionUnion) Add(conditionalAuthorizerName string, d ConditionsAwareDecision) {
	...
	if d.IsConditional() {
		...
		if errs := utilvalidation.IsDNS1123Subdomain(conditionalAuthorizerName); len(errs) != 0 {
			unionMap.errs = append(unionMap.errs, fmt.Errorf("conditionalAuthorizerName %q must be a DNS1123 subdomain, got validation errors: %q", conditionalAuthorizerName, strings.Join(errs, "; ")))
		}
		if strings.Count(conditionalAuthorizerName, ".") < 2 {
			unionMap.errs = append(unionMap.errs, fmt.Errorf("conditionalAuthorizerName %q must have at least two dots", conditionalAuthorizerName))
		}
	}
