...
type evalTestAuthz struct {
	...
	evalErr error
	// conditionalAuthorizerName is used to distinguish authorizers from each other
...
func (a *evalTestAuthz) ConditionalAuthorizerName() string {
	return a.conditionalAuthorizerName
}
...
func TestUnionEvaluateConditions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.authz1.conditionalAuthorizerName = "authz1.example.com"
			tt.authz2.conditionalAuthorizerName = "authz2.example.com"
			tt.authz3.conditionalAuthorizerName = "authz3.example.com"
			tt.authz4.conditionalAuthorizerName = "authz4.example.com"
			tt.authz5.conditionalAuthorizerName = "authz5.example.com"
			union3 := NewConditional("union3.example.com", tt.authz4)
			union2 := NewConditional("union2.example.com", tt.authz1, tt.authz2)
			union1 := NewConditional("union1.example.com", union2, tt.authz3)
			// show that the top-level union does not need to have a name
...
func (c *countingAuthz) ConditionalAuthorizerName() string {
	return c.inner.ConditionalAuthorizerName()
}
...
func TestUnionConditionsAwareAuthorizeShortCircuit(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			authz := New(handlers...)
			condAware, ok := authz.(interface {
				ConditionsAwareAuthorize(context.Context, authorizer.Attributes) authorizer.ConditionsAwareDecision
			})
			if !ok {
				t.Fatalf("union authorizer does not implement ConditionsAwareAuthorize")
			}
			_ = condAware.ConditionsAwareAuthorize(context.Background(), nil)
...
func TestUnionEvaluateConditionsRoutesByAuthorizerName(t *testing.T) {
	...
	first := &evalTestAuthz{conditionEffect: effectAllow, evalDecision: authorizer.DecisionNoOpinion, conditionalAuthorizerName: "first.example.com"}
	second := &evalTestAuthz{conditionEffect: effectAllow, evalDecision: authorizer.DecisionAllow, conditionalAuthorizerName: "second.example.com"}
