...
type SubjectAccessReviewAuthorizationDecision struct {
    ...
    // It does not apply at all to nested conditions in ConditionalDecisionChain.
    //
...
`Status.ConditionalDecisionChain` is mutually exclusive with `Status.Allowed` and
`Status.Denied`. A conditional response is characterized by
`Status.ConditionalDecisionChain != null`. Old implementers that do not recognize
`Status.ConditionalDecisionChain` will just safely assume it was a `NoOpinion`.
...
type AuthorizationConditionsRequest struct {
    ...
    // decisions to other sub-authorizers, might return a ConditionalDecisionChain in
    // SubjectAccessReview. That ConditionalDecisionChain must be sent back unmodified
...
1. If an unconditional response would have been found, `kube-apiserver` would
   ...
   status:
     ...
     conditionalDecisionChain:
     - authorizerName: foo
       conditionalDecisionChain: # nested composite authorizer
       - authorizerName: system
         conditionsType: k8s.io/cel
         conditions:
         - id: foo-system-1
...
- The authorizer implementation supports conditions, which can be done in two ways:
  ...
  - Webhook authorizer: when needed, responds with a non-null
    `.status.conditionalDecisionChain` array, along with `.status.allowed=false` and
    `.status.denied=false`.
